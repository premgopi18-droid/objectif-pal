 ...
 *
 * Entre deux dumps, `series:gcd-live` (#308) ajoute des fascicules et met à
 * jour des séries via l'API comics.org : la bascule les ÉCRASE par les lignes
 * du dump (qui les contient alors) et vide `gcd_series.live_checked_at` (le
 * staging naît par CREATE TABLE LIKE, la colonne n'est pas dans le CSV) —
 * voulu : tout se relit la nuit suivante.
 *
 * Usage :
 *   npm run gcd:load
 *
