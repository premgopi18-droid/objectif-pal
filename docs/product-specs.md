...
sur UNE rangée qui défile au doigt sans barre visible (le retour à la ligne et le scrollbar ont été jugés moches :
patron natif) ; les trois se cumulent, les compteurs d'état disent ce qu'il reste, le bandeau de synthèse reste
celui du parc ; rien n'est persisté) ; la **fiche série** (`?serie=<id>`, plein cadre : grands compteurs,
...
| **Inventaire.io** | ✅ **Ajoutée le 19/07/2026** — projet ouvert (données CC0), API par ISBN sans clé, images communautaires. Point fort mesuré : le fonds **francophone**. **Mesuré le 12/09/2026 (#270)** : leur cache de fichiers pleine taille (`/img/entities/<hash>`) sert des **200 image/webp de 0 octet**, immutables un an, alors que leur redimensionneur (`/img/entities/400x400/<hash>`, ratio préservé) régénère depuis la source → **la variante 400x400 est le chemin fiable** (provider ET rapatriement), l'URL nue le repli. |
