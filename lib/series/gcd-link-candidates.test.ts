...
describe("pickGcdCandidate — nom + famille + année, candidat unique", () => {
  it("Batwoman chez Urban : un seul candidat français chez Urban → unique", () => {
    ...
    expect(verdict).toMatchObject({ kind: "unique", candidate: { id: 68109 } });
  });
  ...
  it("une série GCD plus récente que le plus ancien livre n'est pas la bonne édition", () => {
    ...
    expect(pickGcdCandidate({ seriesName: "Witchblade", publisher: "Delcourt", oldestYear: null, candidates: [candidate(70310, "Witchblade", "Delcourt", { yearBegan: 2012 })] }).kind).toBe("unique");
  });
  ...
  it("éditeur hors table (romans, petites maisons) → rien, raison « no-family »", () => {
    expect(pickGcdCandidate({ seriesName: "Arcane", publisher: "Mana books (Paris)", oldestYear: null, candidates: [candidate(1, "Arcane", "Mana Books")] })).toEqual({
      kind: "none",
      reason: "no-family",
    });
  });
